...
class KMeans<Label: Hashable> {
  ...
  private(set) var centroidsCount = [Int]()
  ...
  func trainCenters(_ points: [Vector], convergeDistance: Double) {
    ...
    repeat {
      ...
      var classification: [[Vector]] = .init(repeating: [], count: numCenters)
        var classificationCount: [Int] = .init(repeating: 0, count: numCenters)
      ...
      for p in points {
        ...
        classification[classIndex].append(p)
          classificationCount[classIndex] += 1
      }
      ...
      centers = newCenters
        countsTracker = classificationCount
    } while centerMoveDist > convergeDistance

    centroids = centers
      centroidsCount = countsTracker
  }
